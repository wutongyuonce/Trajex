...
const CMDS = {
  'this-week': '/trajex recap this week',
  'last-week': '/trajex recap last week',
  'this-month': '/trajex recap this month',
  'last-month': '/trajex recap last month',
};
...
async function loadRecaps() {
  if (!window.trajex?.recapList) return;
  const files = await window.trajex.recapList();
  const results = [];
  for (const f of files) {
    const data = await window.trajex.recapRead(f);
...
onMounted(async () => {
  ...
  if (window.trajex?.onRecapUpdated) {
    unsub = window.trajex.onRecapUpdated(() => loadRecaps());
  }
});
onUnmounted(() => { unsub?.(); });
</script>
            ...
            <div class="empty-row">
              ...
              <div class="empty-cta">
                ...
                <div class="empty-body">
                  Trajex doesn't generate one for you automatically — it waits until you ask. Run <code>/trajex recap this week</code> in Claude Code, and the result will land here as the first marker on this line.
                </div>
    ...
    <div class="modal-backdrop" v-if="showGenerate" @click.self="showGenerate = false">
      <div class="modal">
        ...
        <div class="modal-body">
          <p>Recaps are generated by Claude Code. Run the command below in your terminal — Trajex will pick it up automatically.</p>
