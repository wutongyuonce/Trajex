...
`trajex-skill/references/api-reference.md` and relied on by every example in
`trajex-skill/references/query-patterns.md`. Current behavior is good and there is no reason to
...
`trajex-skill/references/api-reference.md`, so changing a helper forces a doc change plus a
deliberate version bump. `trajex-skill/references/api-reference.md` is therefore promoted from
