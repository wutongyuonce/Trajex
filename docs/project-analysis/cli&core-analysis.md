...
Pi 只支持官方 v3 文件，递归发现。第一行是 `{type:"session",version:3,id,cwd,…}`；其余条目有 `id` 和 `parentId`，并通过 durable `leaf` 指向当前上下文。解析器全量 replay，先校验 entry 身份、父链、leaf 和 retained-tail 结构，再结合 leaf、compaction 的 `firstKeptEntryId` 与 `retainedTail` 重建 active context；字符串缺失父节点是合法 orphan root，其他结构错误让 unit 原子失败。多次 compaction 时，反向追溯在最近的 retained-tail checkpoint 停止，再在这段有效 path 内应用最后一次 compaction；retained tail 只投影一次并重连后续消息。工具调用按具体 message occurrence 生成 canonical ID，结果只解析同一 `parentId` 分支的最近 native ID，避免 fork 复用 ID 时覆盖或串线。当前上下文投影为 `visible`，已被分支替代或压缩的物理证据保留为 `inactive`，来源明确不展示的 custom message 投影为 `hidden`。
...
jsonl_path = "__claude_canonical_transcript_v5__" / "__codex_canonical_transcript_v6__" / "__pi_canonical_transcript_v9__"
              ↑ 不是路径，而是状态 key
```
